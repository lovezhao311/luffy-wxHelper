...
use luffyzhao\wxhelper\library\http\Response;
    ...
    private function checkResponse($ch)
    {
        ...
        if ($this->failOnError) {
            ...
            if ($status >= 400 && $status <= 499) {
                throw new ClientException($this->getStatusMessage(), $status, $this->getResponse());
            } elseif ($status >= 500 && $status <= 599) {
                throw new ServerException($this->getStatusMessage(), $status, $this->getResponse());
            }
        }
    ...
    private function followRedirectPath()
    {
        ...
        if ($this->getStatus() == 301 || $this->getStatus() == 302) {
            ...
            if ($this->redirectsFollowed < $this->maxRedirects) {
                ...
                throw new NetworkException($errorString, CURLE_TOO_MANY_REDIRECTS);
            }
        } else {
